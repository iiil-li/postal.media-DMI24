...
reusability="restricted%2Copen"
media="true"
rows="100"
wskey="[INSERT API KEY]"
qf="DATA_PROVIDER%3APostmuseum"

# Function to urlencode using jq
urlencode() {
    # Use jq to JSON encode, remove surrounding quotes, replace characters with % encoding
    jq -rn --arg v "$1" '$v | @uri' | sed 's/^"\(.*\)"$/\1/'
}
...
while [ "$cursor" != "null" ]; do
    ...
    target_url="${base_url}?query=${encoded_query}&reusability=${reusability}&qf=${qf}&cursor=${cursor}&media=${media}&rows=${rows}&wskey=${wskey}"
